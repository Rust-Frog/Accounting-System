...
(function () {
    ...
    const elements = {
        ...
        // Period tabs and content
        modalTabs: document.querySelector('.modal-tabs'),
        tabBtns: document.querySelectorAll('.tab-btn'),
        tabContents: document.querySelectorAll('.tab-content'),
        periodsBody: document.getElementById('periodsBody'),
        btnClosePeriod: document.getElementById('btnClosePeriod'),
        // Close period modal
        closePeriodModal: document.getElementById('closePeriodModal'),
        closePeriodForm: document.getElementById('closePeriodForm'),
        btnClosePeriodModal: document.getElementById('btnClosePeriodModal'),
        btnCancelClosePeriod: document.getElementById('btnCancelClosePeriod'),
        periodStartDate: document.getElementById('periodStartDate'),
        periodEndDate: document.getElementById('periodEndDate'),
        btnSubmitClosePeriod: document.getElementById('btnSubmitClosePeriod'),
        // Confirm modal
    ...
    async function openDetailsModal(companyId) {
        ...
        switchTab('overview');
        elements.detailsContent.querySelector('#tab-overview').innerHTML = '<div class="loading-spinner">Loading...</div>';

        try {
            const response = await api.get(`/companies/${companyId}`);
            currentCompany = response.data;
            renderCompanyDetails(currentCompany);
            await loadClosedPeriods(companyId);
        } catch (error) {
            console.error('Failed to load company details:', error);
            elements.detailsContent.querySelector('#tab-overview').innerHTML = `
    ...
    tbody.innerHTML = periods.map(period => `
                <tr>
                    ...
                    <td>
                        <div class="text-sm text-secondary">ID: ${period.closed_by.substring(0, 8)}...</div>
                    </td>
                    <td>${formatDateTime(period.closed_at)}</td>
                    <td class="text-right font-mono">
                        ${formatCurrency(period.net_income_cents / 100, currentCompany.currency)}
                    </td>
                </tr>
            `).join('');
